...
from rsl_rl.configs import LeggedRobotCfgDDPG, envCfg
from rsl_rl.utils import class_to_dict

def train():

    cfg = FrankaReachCfg()
    train_cfg = class_to_dict(LeggedRobotCfgDDPG())
    ...
    ddpg_runner = OffPolicyRunner(env=env, train_cfg=train_cfg, log_dir='logs1', device=str(env.device))
    ddpg_runner.learn(num_learning_iterations=train_cfg["runner"]["max_iterations"], init_at_random_ep_len=True)
