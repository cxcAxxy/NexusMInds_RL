...
class Reach_random_points(Task):
    def __init__(self, sim, cfg) -> None:
        ...
        # 目标范围
        self.goal_range_low = torch.tensor([-cfg.goal_range / 2, -cfg.goal_range / 2, 0.2],
                                           dtype=torch.float32, device=self.device)
        self.goal_range_high = torch.tensor([cfg.goal_range / 2, cfg.goal_range / 2, cfg.goal_range / 2+0.2],
                                            dtype=torch.float32, device=self.device)
